...
void Sherlock::ExploreTopNode(
    ...
    for (int link_id = data->inverse_links.size() - 1; link_id > max_link_id;
        ...
        if (new_hypothesis->size() < MAX_FAILS) {
            ...
            UpdateScores(*new_scores, new_hypothesis, base_hypothesis,
            ...
            ExploreTopNode(min_start_time_ms, max_finish_time_ms,
                           all_hypothesis, nopenmp_threads, hstack);
            if (++num_hypothesis_finished % 5000 == 0)
                cout << "Finished " << num_hypothesis_finished
                     << " hypothesis in "
                     << GetTimeSinceSeconds(timer_checkpoint) << " seconds "
                     << endl;
        }
        // else cout << "new_hypothesis " << *new_hypothesis << " " <<
        // new_likelihood << " " << *base_hypothesis << " " << base_likelihood
        // << endl;
    }
    sort(result.begin(), result.end(), greater<pair<double, Hypothesis *>>());
    // int nsave = (base_hypothesis->size() < MAX_FAILS-1? result.size():
    // min(10, (int)result.size()));
    int nsave =
        (base_hypothesis->size() < MAX_FAILS - 1 ? 0
                                                 : min(10, (int)result.size()));
...
void Sherlock::SearchHypothesesFlock(
    double min_start_time_ms, double max_finish_time_ms,
    unordered_map<Hypothesis *, double> &all_hypothesis, int nopenmp_threads) {
    ...
    ExploreTopNode(min_start_time_ms, max_finish_time_ms, all_hypothesis,
                   nopenmp_threads, hstack);
}

void Sherlock::SearchHypotheses(
    double min_start_time_ms, double max_finish_time_ms,
    unordered_map<Hypothesis *, double> &all_hypothesis, int nopenmp_threads) {
    Hypothesis *no_failure_hypothesis = new Hypothesis();
    all_hypothesis[no_failure_hypothesis] = 0.0;
    ...
    while (nfails <= MAX_FAILS) {
        ...
        for (Hypothesis *h : prev_hypothesis_space) {
            ...
            int nsave = (nfails < MAX_FAILS ? result.size()
                                            : min(10, (int)result.size()));
            int curr = 0;
            while (curr < nsave) {
                all_hypothesis[result[curr].second] = result[curr].first;
            ...
            if (++ctr % 5 == 0)
                cout << "Finished hypothesis search across "
                     << hypothesis_analyzed << " Hypothesis for " << nfails
                     << " failures in " << GetTimeSinceSeconds(start_stage_time)
                     << " seconds" << endl;
        }
        if (VERBOSE) {
            cout << "Finished hypothesis search across " << hypothesis_analyzed
                 << " Hypothesis for " << nfails << " failures in "
                 << GetTimeSinceSeconds(start_stage_time) << " seconds" << endl;
        }
        for (Hypothesis *h : prev_hypothesis_space) {
            if (all_hypothesis.find(h) == all_hypothesis.end())
                delete (h);
        }
        prev_hypothesis_space.clear();
        prev_hypothesis_space.insert(prev_hypothesis_space.begin(),
                                     next_hypothesis_space.begin(),
                                     next_hypothesis_space.end());
        nfails++;
    }
    cout << "Finished: " << all_hypothesis[no_failure_hypothesis] << " "
         << all_hypothesis.size() << endl;
}

void Sherlock::LocalizeFailures(double min_start_time_ms,
    ...
    if (VERBOSE) {
        ...
        cout << "start_time_ms " << min_start_time_ms << " "
    ...
    if (VERBOSE and PRINT_SCORES) {
        ...
        PrintScores(min_start_time_ms, max_finish_time_ms, false,
                    nopenmp_threads);
    ...
    auto start_search_time = chrono::high_resolution_clock::now();
    SearchHypotheses(min_start_time_ms, max_finish_time_ms, all_hypothesis,
                     nopenmp_threads);
    ...
    for (int i =
             min((int)likelihood_hypothesis.size(), N_MAX_K_LIKELIHOODS) - 1;
         i >= 0; i--) {
        double likelihood = likelihood_hypothesis[i].first;
        Hypothesis *hypothesis = likelihood_hypothesis[i].second;
        if (highest_likelihood - likelihood <= 1.0e-3 and
            candidate_hypothesis.size() == 0) {
    ...
    for (int link_id : erase_link_ids)
        candidate_link_ids.erase(link_id);
    localized_links.insert(candidate_link_ids.begin(),
                           candidate_link_ids.end());
    ...
    if (VERBOSE) {
        ...
        Hypothesis *no_failure_hypothesis = new Hypothesis();
        double likelihood_correct_hypothesis = ComputeLogLikelihood(
        ...
        if (PRINT_SCORES) {
            for (int link_id : localized_links) {
                ...
                double l = ComputeLogLikelihood(
                    &single_link_hypothesis, no_failure_hypothesis, 0.0,
                    min_start_time_ms, max_finish_time_ms, nopenmp_threads);
                // cout << "Score of predicted failed link " <<
                // data->inverse_links[link_id] << " score " <<
                // drops_per_link[link_id] << " E[nflows] " <<
                // flows_per_link[link_id] << " likelihood " << l << endl;
            }
        }
