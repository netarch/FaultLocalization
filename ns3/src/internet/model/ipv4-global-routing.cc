...
void
Ipv4GlobalRouting::LogFlowRule (const Ipv4Header &header, Ptr<const Packet> ipPayload, std::vector<Ipv4RoutingTableEntry*> &allRoutes, uint32_t selectIndex){
  uint32_t sourcePort, destinationPort, protocol;
  ...
  FlowRule rule = FlowRule(sourcePort, destinationPort, header.GetSource().Get(), header.GetDestination().Get(), protocol, selectIndex);
  if (flowrules.find(rule) != flowrules.end()){
    //auto it  = flowrules.find(rule);
    //assert(*it == rule);
    return;
  }
