...
void BayesianNetContinuous::SetLogData(LogData *data_,
...
bool BayesianNetContinuous::DiscardFlow(Flow *flow, double min_start_time_ms,
                                        double max_finish_time_ms) {
    return (flow->start_time_ms < min_start_time_ms or
            !flow->AnySnapshotBefore(max_finish_time_ms) or
            ((INPUT_FLOW_TYPE == PROBLEMATIC_FLOWS) and
             !flow->TracerouteFlow(max_finish_time_ms)));
}

long double
BayesianNetContinuous::FlowProbabilityForPath(int weight_good, int weight_bad,
...
long double
BayesianNetContinuous::PathGradientNumerator(int weight_good, int weight_bad,
                                             double path_fail_rate) {
    if (weight_bad == 0 and weight_good == 0)
        return 0.0;
    else if (weight_bad == 0)
        return -weight_good *
               pow((long double)(1.0 - path_fail_rate), weight_good - 1);
    else if (weight_good == 0)
        return weight_bad * pow((long double)path_fail_rate, weight_bad - 1);
    else
        return pow((long double)path_fail_rate, weight_bad - 1) *
               pow((long double)(1.0 - path_fail_rate), weight_good - 1) *
               (weight_bad * (1.0 - path_fail_rate) -
                weight_good * path_fail_rate);
}

double BayesianNetContinuous::PathFailRate(int first_link_id, int last_link_id,
                                           Path *path,
                                           vector<double> &loss_rates) {
    double path_success_rate =
        (1.0 - loss_rates[first_link_id]) * (1.0 - loss_rates[last_link_id]);
...
void BayesianNetContinuous::LocalizeFailures(double min_start_time_ms,
                                             double max_finish_time_ms,
    ...
    while (++iter < niters and delta >= -1.0e2) {
        ...
        log_likelihood = ComputeLogLikelihood(
            loss_rates, min_start_time_ms, max_finish_time_ms, nopenmp_threads);
        delta += log_likelihood;
        int nflows = ComputeGradients(gradients, loss_rates, min_start_time_ms,
                                      max_finish_time_ms, nopenmp_threads);
        localized_links.clear();
        double decayed_learning_rate =
            learning_rate / (1.0 + decay_rate * iter);
        for (int link_id = 0; link_id < nlinks; link_id++) {
            // cout << "Gradient " << gradients[link_id] << endl;
            // moving_gradients[link_id] = moving_gradients[link_id] *
            // moving_average_weight
            //                          + gradients[link_id] * (1.0 -
            //                          moving_average_weight);
            double del = -loss_rates_nav[link_id];
            loss_rates_nav[link_id] =
                loss_rates[link_id] +
                (decayed_learning_rate * gradients[link_id]) / nflows;
        ...
        if (VERBOSE and iter % 20 == 0) {
            PDD precision_recall =
                GetPrecisionRecall(failed_links_set, localized_links, data);
            cout << "Iteration " << iter << " log likelihood " << log_likelihood
                 << " delta " << delta << " learning rate "
                 << decayed_learning_rate << " hypothesis "
                 << data->IdsToLinks(localized_links) << " pr "
                 << precision_recall << " in time "
                 << GetTimeSinceSeconds(start_iter_time) << endl;
        }
    }
    for (int link_id : localized_links) {
        cout << "Predicted failed link " << data->inverse_links[link_id]
             << " loss rate " << loss_rates[link_id] << endl;
    }
}

inline long double BayesianNetContinuous::ComputeLogPrior(double loss_rate) {
    return log(1.0 / (1.0 + exp((long double)sigmoid_constant *
                                (loss_rate - loss_rate_mid))));
}

inline long double
BayesianNetContinuous::ComputeLogPriorGradient(double loss_rate) {
    return -sigmoid_constant / (1.0 + exp((long double)-sigmoid_constant *
                                          (loss_rate - loss_rate_mid)));
}

long double BayesianNetContinuous::ComputeLogLikelihood(
...
#pragma omp parallel for num_threads(nopenmp_threads)
    for (int ff = 0; ff < data->flows.size(); ff++) {
        ...
        for (Path *path : *flow_paths) {
            double path_fail_rate = PathFailRate(
                flow->first_link_id, flow->last_link_id, path, loss_rates);
            flow_prob += FlowProbabilityForPath(weight.first, weight.second,
                                                path_fail_rate) /
                         npaths;
        }
    ...
    long double log_likelihood =
        accumulate(likelihood_threads.begin(), likelihood_threads.end(), 0.0,
                   plus<double>());
    if (APPLY_PRIOR) {
        for (int link_id = 0; link_id < nlinks; link_id++) {
...
#pragma omp parallel for num_threads(nopenmp_threads)
    for (int ff = 0; ff < data->flows.size(); ff++) {
        ...
        for (Path *path : *flow_paths) {
            double path_fail_rate = PathFailRate(
                flow->first_link_id, flow->last_link_id, path, loss_rates);
            flow_prob1 += FlowProbabilityForPath(weight.first, weight.second,
                                                 path_fail_rate) /
                          npaths;
            long double gradient_common = PathGradientNumerator(
                weight.first, weight.second, path_fail_rate);
            gradient_common *= (1.0 - path_fail_rate);
            gradient_common /= flow_prob;
            gradient_common /= npaths;
            gradients_threads[thread_num][flow->first_link_id] +=
                gradient_common / (1.0 - loss_rates[flow->first_link_id]);
            gradients_threads[thread_num][flow->last_link_id] +=
                gradient_common / (1.0 - loss_rates[flow->last_link_id]);
            for (int link_id : *path) {
                gradients_threads[thread_num][link_id] +=
                    gradient_common / (1.0 - loss_rates[link_id]);
            }
        }
...
#pragma omp parallel for num_threads(nopenmp_threads)
    for (int link_id = 0; link_id < nlinks; link_id++) {
        //! TODO: Try changing 2d array from (threads X links) to (links X
        //! threads)
    ...
    return accumulate(nflows_threads.begin(), nflows_threads.end(), 0,
                      plus<int>());
}
