    ...
    case CLOSED:
      ...
      if ((tcpHeader.GetFlags () & ~(TcpHeader::PSH | TcpHeader::URG)) != TcpHeader::RST)
        { // Since m_endPoint is not configured yet, we cannot use SendRST here
          ...
          std::cout<<"? " <<tcpHeader.GetDestinationAddress ()<<" "<<tcpHeader.GetSourceAddress ()<<std::endl;
  ...
  if (m_endPoint != 0)
    {
      ...
      header.SetSourceAddress(m_endPoint->GetLocalAddress ());
      header.SetDestinationAddress(m_endPoint->GetPeerAddress ());
    }
  else
    {
  ...
  if (m_endPoint != 0)
    {
      ...
      tcpHeader.SetSourceAddress(m_endPoint->GetLocalAddress ());
      tcpHeader.SetDestinationAddress(m_endPoint->GetPeerAddress ());
    }
  else
    {
...
uint32_t
TcpSocketBase::GetSentPackets (void) const
{
  return m_sentPackets;
}

uint32_t
TcpSocketBase::GetLostPackets (void) const
{
...
uint32_t
TcpSocketBase::GetLocalPort (void) const
{
  //Assumes IPv4
...
uint32_t
TcpSocketBase::GetPeerPort (void) const
{
  //Assumes IPv4
...
Ipv4Address
TcpSocketBase::GetLocalAddress (void) const
{
  //Assumes IPv4
...
Ipv4Address
TcpSocketBase::GetPeerAddress (void) const
{
  //Assumes IPv4
  return m_endPoint->GetPeerAddress();
}
