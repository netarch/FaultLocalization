...
void Score::SetLogData(LogData* data_, double max_finish_time_ms, int nopenmp_threads){
...
PDD Score::ComputeScoresPassive(vector<double>& scores, Hypothesis &problematic_link_ids,
    ...
    for(int ff=0; ff < data->flows.size(); ff++){
        ...
        double score1 = ((double) flow->LabelWeightsFunc(max_finish_time_ms).second);
        double score2 = ((double) flow->LabelWeightsFunc(max_finish_time_ms).first);
    ...
    for (int i=0; i<nlinks; i++) scores[i] = scores1[i]/max(1.0e-9, scores2[i]);
    double sum_scores = accumulate(scores.begin(), scores.end(), 0.0, plus<double>());
    ...
    */
    for(int ff=0; ff < bad_flows.size(); ff++){
        ...
        if (!HypothesisIntersectsPath(&problematic_link_ids, path_taken) and
            (problematic_link_ids.count(flow->first_link_id) == 0) and
            (problematic_link_ids.count(flow->last_link_id) == 0)){
            int flow_score = (int) (flow->LabelWeightsFunc(max_finish_time_ms).second > 0);
            assert(flow_score == 1);
            int total_path_length = path_taken->size() + 2;
            ...
            double score1 = ((double) flow->LabelWeightsFunc(max_finish_time_ms).second);
            double score2 = ((double) flow->LabelWeightsFunc(max_finish_time_ms).first);
            scores1[flow->first_link_id] += score1; scores2[flow->first_link_id] += score2;
            scores1[flow->last_link_id] += score1; scores2[flow->last_link_id] += score2;
            */
...
void Score::LocalizeFailures(double min_start_time_ms, double max_finish_time_ms,
    ...
    if (USE_PASSIVE){
        tie(sum_scores, max_scores) = ComputeScoresPassive(scores, localized_links,
    ...
    while ((USE_ABSOLUTE_THRESHOLD and (max_scores >= fail_threshold)) or
        ...
        if(ADJUST_VOTES){
            ...
            scores[faulty_link_id] = -1000000; // Ensure it doesn't show up again
