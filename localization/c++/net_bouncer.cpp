...
void NetBouncer::SetLogData(LogData *data_, double max_finish_time_ms,
                            int nopenmp_threads) {
...
NetBouncer *NetBouncer::CreateObject() {
    ...
    vector<double> param{regularize_const, fail_threshold,
                         bad_device_frac_bad_flows_threshold};
    ret->SetParams(param);
    return ret;
}
...
double NetBouncer::EstimatedPathSuccessRate(Flow *flow,
                                            vector<double> &success_prob) {
    Path *path_taken = flow->GetPathTaken();
    double x =
        success_prob[flow->first_link_id] * success_prob[flow->last_link_id];
...
void NetBouncer::RemoveBadDevices(double min_start_time_ms,
    ...
    for (int device = 0; device < data->GetMaxDevicePlus1(); device++) {
        ...
        if (VERBOSE and (double) bad_flows / device_flows.size() >
                            0.125 * bad_device_frac_bad_flows_threshold)
            cout << "Device " << device << ", bad_flows: " << bad_flows << "/"
                 << device_flows.size() << endl;
        if ((double)bad_flows / device_flows.size() >
            bad_device_frac_bad_flows_threshold)
            bad_devices.insert(device);
    }
    if (VERBOSE)
    ...
    for (int link_id = 0; link_id < nlinks; link_id++) {
        ...
        bad_device_links[link_id] =
            ((bad_devices.find(link.first) != bad_devices.end()) or
             (bad_devices.find(link.second) != bad_devices.end()));
    }
}

void NetBouncer::DetectBadDevices(vector<int> &bad_devices,
    ...
    for (int device = 0; device < data->GetMaxDevicePlus1(); device++) {
        ...
        cout << "Device " << device << ", num_flows: " << device_flows.size()
             << " : " << good_device << endl;
        if (!good_device)
            bad_devices.push_back(device);
    }
}

double NetBouncer::ComputeError(vector<Flow *> &active_flows,
                                vector<double> &success_prob,
                                double min_start_time_ms,
                                double max_finish_time_ms) {
    double error = 0.0;
    ...
    for (int ff = 0; ff < active_flows.size(); ff++) {
        ...
        double y = 1.0 - flow->GetDropRate(max_finish_time_ms);
        double multiplier =
            (USE_MULTIPLIER ? flow->GetPacketsSent(max_finish_time_ms) : 1);
        error += multiplier * (y - x) * (y - x);
        // cout << error << " " << y << " " << x << " " <<
        // flow->GetPacketsSent(max_finish_time_ms)
        //     << " " << flow->GetPacketsLost(max_finish_time_ms) << " "
        //     << flow->GetDropRate(max_finish_time_ms) << endl;
    }
    for (int link_id = 0; link_id < nlinks; link_id++) {
        error += regularize_const * success_prob[link_id] *
                 (1.0 - success_prob[link_id]);
    }
    return error;
}
...
double NetBouncer::ArgMinError(vector<double> &success_prob, int var_link_id,
    ...
    // bool verbose = false; Link l(0, 10013); if
    // (data->inverse_links[var_link_id] == l) verbose = true;
    // //no_failure/test_random_0_15
    for (int ff : link_id_flows) {
        Flow *flow = data->flows[ff];
        // assert(flow->TracerouteFlow(max_finish_time_ms));
        // Path *path_taken = flow->GetPathTaken();
        // if (path_taken->find(var_link_id)!=path_taken->end() or
        // flow->first_link_id == var_link_id
        //   or flow->last_link_id == var_link_id){
        double multiplier =
            (USE_MULTIPLIER ? flow->GetPacketsSent(max_finish_time_ms) : 1);
        double t = EstimatedPathSuccessRate(flow, success_prob) /
                   success_prob[var_link_id];
        double y = 1.0 - flow->GetDropRate(max_finish_time_ms);
        s1 += t * y * multiplier;
        s2 += t * t * multiplier;
...
void NetBouncer::LocalizeFailures(double min_start_time_ms,
                                  double max_finish_time_ms,
    ...
    for (Flow *flow : data->flows) {
        ...
        for (int link_id : *path_taken)
            bad_device_in_path =
                bad_device_in_path or bad_device_links[link_id];
    ...
    // cout << "numflows " << active_flows.size() << " max_finish_time_ms " <<
    ...
    for (int ff = 0; ff < active_flows.size(); ff++) {
        ...
        // cout << ff << " " << flow->GetPacketsSent(max_finish_time_ms) << " "
        // << flow_drop_rate << endl;
        if (flow->GetPacketsSent(max_finish_time_ms) == 0)
            continue;
    ...
    for (int link_id = 0; link_id < nlinks; link_id++) {
        if (!bad_device_links[link_id] and !data->IsLinkDevice(link_id)) {
            ...
            success_prob[link_id] /=
                max(1.0e-30, num_flows_through_link[link_id]);
        }
    }
    double error = ComputeError(active_flows, success_prob, min_start_time_ms,
                                max_finish_time_ms);
    for (int it = 0; it < MAX_ITERATIONS; it++) {
        ...
        for (int link_id = 0; link_id < nlinks; link_id++) {
            if (!bad_device_links[link_id] and !data->IsLinkDevice(link_id))
                success_prob[link_id] =
                    max(0.0, min(1.0, ArgMinError(success_prob, link_id,
                                                  min_start_time_ms,
                                                  max_finish_time_ms)));
        }
        double new_error = ComputeError(active_flows, success_prob,
                                        min_start_time_ms, max_finish_time_ms);
        if (VERBOSE) {
            cout << "Iteration " << it << " error " << new_error
                 << " finished in " << GetTimeSinceSeconds(start_iter_time)
                 << endl;
        }
        if (abs(new_error - error) < 1.0e-8)
            break;
        error = new_error;
    }

    localized_links.clear();
    for (int d : bad_devices)
        localized_links.insert(data->GetLinkIdUnsafe(Link(d, d)));

    if (MISCONFIGURED_ACL) {
        vector<pair<double, int>> success_prob_link;
        for (int link_id = 0; link_id < nlinks; link_id++) {
            if (!bad_device_links[link_id] and !data->IsLinkDevice(link_id))
                success_prob_link.push_back(
                    pair<double, int>(success_prob[link_id], link_id));
        }
        sort(success_prob_link.begin(), success_prob_link.end());
        for (int ii = 0;
             ii < min((int)success_prob_link.size(), NUM_HYPOTHESIS_FOR_UNION);
             ii++) {
            auto [sprob, link_id] = success_prob_link[ii];
            if (localized_links.size() < NUM_HYPOTHESIS_FOR_UNION and
                (*flows_by_link_id)[link_id].size() > 0) {
        ...
        for (int link_id = 0; link_id < nlinks; link_id++) {
            if (!bad_device_links[link_id] and !data->IsLinkDevice(link_id)) {
                if (VERBOSE and
                    (1.0 - success_prob[link_id] >= fail_threshold / 10)) {
                    cout << "Suspicious link " << data->inverse_links[link_id]
                         << " " << 1.0 - success_prob[link_id] << " "
                         << fail_threshold << endl;
                }
                if (1.0 - success_prob[link_id] >= fail_threshold and
                    (*flows_by_link_id)[link_id].size() > 0) {
                    localized_links.insert(link_id);
                }
            }
