...
void DoubleO7::SetLogData(LogData *data_, double max_finish_time_ms,
                          int nopenmp_threads) {
...
PDD DoubleO7::ComputeVotes(vector<Flow *> &bad_flows, vector<double> &votes,
    ...
    for (int ff = 0; ff < bad_flows.size(); ff++) {
        ...
        if (!HypothesisIntersectsPath(&problematic_link_ids, path_taken) and
            ...
            // cout << " path_taken " << *path_taken << " " << flow_vote <<
            // endl;
            if (FILTER_NOISY_DROPS) {
                flow_vote =
                    (int)(flow->LabelWeightsFunc(max_finish_time_ms).second >
                          1);
            } else {
            ...
            if constexpr (USE_BUGGY_007)
                vote = ((double)flow_vote) / (total_path_length - 1);
    ...
    double sum_votes =
        accumulate(votes.begin(), votes.end(), 0.0, plus<double>());
    double max_votes = *max_element(votes.begin(), votes.end());
    return PDD(sum_votes, max_votes);
}

void DoubleO7::LocalizeFailures(double min_start_time_ms,
                                double max_finish_time_ms,
...
// Vote adjustment
void DoubleO7::LocalizeLinkFailures(double min_start_time_ms,
                                    double max_finish_time_ms,
    ...
    auto [sum_votes, max_votes] =
        ComputeVotes(bad_flows, votes, localized_links, min_start_time_ms,
                     max_finish_time_ms);
    if (VERBOSE) {
    ...
    for (int link_id=0; link_id<nlinks; link_id++){
        ...
        cout << (link.first > 10000? link.first - 10000 : link.first) << " "
             << (link.second > 10000? link.second - 10000: link.second) << " "
    << votes[link_id] << endl;
    }
    return;
    */
    int iter = 0;
    while ((MISCONFIGURED_ACL and
            localized_links.size() < NUM_HYPOTHESIS_FOR_UNION)
           // or (iter < MAX_ITER)){
           or (max_votes >= fail_threshold * sum_votes and max_votes > 0)) {
        ++iter;
        int faulty_link_id =
            distance(votes.begin(), max_element(votes.begin(), votes.end()));
        localized_links.insert(faulty_link_id);
        if (ADJUST_VOTES) {
            tie(sum_votes, max_votes) =
                ComputeVotes(bad_flows, votes, localized_links,
...
PDD DoubleO7::ComputeVotesDevice(vector<Flow *> &bad_flows,
                                 vector<double> &votes,
...
void DoubleO7::LocalizeDeviceFailures(double min_start_time_ms,
                                      double max_finish_time_ms,
    ...
    auto [sum_votes, max_votes] =
        ComputeVotesDevice(bad_flows, votes, localized_devices,
                           min_start_time_ms, max_finish_time_ms);
    if (VERBOSE) {
        cout << " sumvote " << sum_votes << " maxvote " << max_votes
             << " numflows " << bad_flows.size() << endl;
    ...
    int iter = 0;
    while //(iter < MAX_ITER){
        (max_votes >= fail_threshold * sum_votes and max_votes > 0) {
        ++iter;
        int faulty_device =
            distance(votes.begin(), max_element(votes.begin(), votes.end()));
    ...
    for (int device : localized_devices)
        localized_device_links.insert(
            data->GetLinkIdUnsafe(Link(device, device)));
}
