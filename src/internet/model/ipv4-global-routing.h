...
struct FlowRule{
  uint32_t sourcePort, destinationPort;
  uint32_t sourceIp, destinationIp;
  uint32_t protocol, interface;
  FlowRule(uint32_t _sourcePort, uint32_t _destinationPort, uint32_t _sourceIp, 
            uint32_t _destinationIp, uint32_t _protocol, uint32_t _interface){
  ...
  bool operator==(const FlowRule& rhs) const{
      return ((sourcePort == rhs.sourcePort)
          &&  (destinationPort == rhs.destinationPort)
          &&  (sourceIp == rhs.sourceIp)
          &&  (destinationIp == rhs.destinationIp)
...
private:
  ...
  std::unordered_set<FlowRule, HashFunction_FlowRule> flowrules; //!< hash table of all rules

  void LogFlowRule (const Ipv4Header &header, Ptr<const Packet> ipPayload, std::vector<Ipv4RoutingTableEntry*> &allRoutes, uint32_t selectIndex);
};
} // Namespace ns3
