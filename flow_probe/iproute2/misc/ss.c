...
#define INTERVAL_SS 10000
static unsigned long long ss_index = 0;
struct Ipv4TcpFlowEntry* flow_hash_map[DEFAULT_HASH_SIZE];
struct FlowEntryWrapper* active_flows_front = NULL;
struct FlowEntryWrapper* active_flows_back = NULL;
struct Ipv4TcpFlowEntry* flow_stats_list_front = NULL;
struct Ipv4TcpFlowEntry* flow_stats_list_back = NULL;
...
static void sock_state_print(struct sockstat *s)
{
	...
	case AF_INET:
	case AF_INET6:
		// sock_name default to "tcp", since our net-flow agent only cares TCP.
...
static void tcp_show_info(const struct sockstat *sock_stat,
													const struct nlmsghdr *nlh,
													struct inet_diag_msg *r,
													struct rtattr *tb[])
{
	unsigned int src_inet_prefix = (unsigned int)*(sock_stat->local.data);
	unsigned int dst_inet_prefix = (unsigned int)*(sock_stat->remote.data);
	unsigned int src_ip = (src_inet_prefix & 0xff) << 24;
	src_ip += ((src_inet_prefix >> 8) & 0xff) << 16;
	src_ip += ((src_inet_prefix >> 16) & 0xff) << 8;
	src_ip += ((src_inet_prefix >> 24) & 0xff);
	unsigned int dst_ip = (dst_inet_prefix & 0xff) << 24;
	dst_ip += ((dst_inet_prefix >> 8) & 0xff) << 16;
	dst_ip += ((dst_inet_prefix >> 16) & 0xff) << 8;
	dst_ip += ((dst_inet_prefix >> 24) & 0xff);
	unsigned int pkt_hash = src_ip + dst_ip + sock_stat->lport + sock_stat->rport;
	int hash_index = pkt_hash % DEFAULT_HASH_SIZE;
	//char sbuf[256], rbuf[256];
	//inet_ntop(AF_INET, sock_stat->local.data, sbuf, 256);
	//inet_ntop(AF_INET, sock_stat->remote.data, rbuf, 256);
	//printf("-----------------%s(%u):%d -> %s(%u):%d (%u)(%d)\n",
	//		sbuf, src_ip, sock_stat->lport,
	//		rbuf, dst_ip, sock_stat->rport, pkt_hash, hash_index);
	...
	if (tb[INET_DIAG_INFO]) {
		...
		while (entry != NULL) {
			if (entry->src_ip == src_ip && entry->dst_ip == dst_ip &&
					entry->src_port == (u_int16_t)sock_stat->lport &&
					entry->dst_port == (u_int16_t)sock_stat->rport) {
				break;
			} else if (entry->version < ss_index - 1) {
				struct Ipv4TcpFlowEntry* tmp_entry = entry;
				entry = entry->next;
				if (entry != NULL) {
					entry->prev = prev;
				}
		...
		if (entry == NULL) {
			...
			//		sbuf, sock_stat->lport, rbuf, sock_stat->rport);
			...
			entry->src_port = (u_int16_t)sock_stat->lport;
			entry->dst_port = (u_int16_t)sock_stat->rport;
			entry->out_bytes_snapshot = 0;
			entry->out_packets_snapshot = 0;
			entry->retrans_bytes_snapshot = 0;
			entry->retrans_packets_snapshot = 0;
			entry->prev = prev;
			entry->next = NULL;
			entry->version = ss_index;
			...
			struct FlowEntryWrapper* entry_wrapper =
					(struct FlowEntryWrapper*)malloc(sizeof(struct FlowEntryWrapper));
			entry_wrapper->entry = entry;
			entry_wrapper->next = NULL;

			if (active_flows_front == NULL) {
				active_flows_front = entry_wrapper;
				active_flows_back = entry_wrapper;
			} else {
				active_flows_back->next = entry_wrapper;
				active_flows_back = entry_wrapper;
			}
		}
		entry->out_bytes = info->tcpi_bytes_sent;
		entry->out_packets = info->tcpi_data_segs_out;
		entry->retrans_bytes = info->tcpi_bytes_retrans;
		entry->retrans_packets = info->tcpi_total_retrans;
		entry->srtt_usec = info->tcpi_rtt;
		if (entry->version >= ss_index - 1) {
			entry->version = ss_index;
		}
	}
}
...
"\n"
"   -j                  interval of export in microseconds\n"
"   -z                  ip address of IPFIX collector\n"
"   -x                  listening port of IPFIX collector\n"
"\n"
...
void update_next_export_ts(void) {
...
int byte_to_string(unsigned int byte, char* addr, int pos) {
...
void ip_addr_binary_to_string(unsigned int ip, char* addr) {
	int pos = 0;
	pos = byte_to_string((ip >> 24) & 0xff, addr, pos);
	addr[pos++] = '.';
	pos = byte_to_string((ip >> 16) & 0xff, addr, pos);
	addr[pos++] = '.';
	pos = byte_to_string((ip >> 8) & 0xff, addr, pos);
	addr[pos++] = '.';
	pos = byte_to_string(ip & 0xff, addr, pos);
	addr[pos] = '\0';
}
...
void create_export_socket(void) {
...
void encapsulate_flow_record(char* buffer, struct Ipv4TcpFlowEntry* entry) {
	...
	memcpy(buffer + 28, &entry->srtt_usec, sizeof(u_int32_t));
	memcpy(buffer + 32, &entry->srtt_usec, sizeof(u_int32_t));

	// No ts available from ss output.
	u_int32_t ts = (u_int32_t)0;
...
void maybe_prepare_export(void) {
	struct FlowEntryWrapper* entry_wrapper = active_flows_front;
	struct FlowEntryWrapper* prev_wrapper = NULL;
	struct Ipv4TcpFlowEntry* entry = NULL;
	while (entry_wrapper != NULL) {
		entry = entry_wrapper->entry;

		//unsigned int src_ip = entry->src_ip;
		//unsigned int dst_ip = entry->dst_ip;
		//int src_port = entry->src_port;
		//int dst_port = entry->dst_port;

		//char src_ip_addr[256];
		//ip_addr_binary_to_string(src_ip, src_ip_addr);
		//char dst_ip_addr[256];
		//ip_addr_binary_to_string(dst_ip, dst_ip_addr);

		if (entry->version < ss_index) {
			u_int32_t out_bytes = entry->out_bytes - entry->out_bytes_snapshot;
			u_int32_t out_packets = entry->out_packets - entry->out_packets_snapshot;
			u_int32_t retrans_bytes =
					entry->retrans_bytes - entry->retrans_bytes_snapshot;
			u_int32_t retrans_packets =
					entry->retrans_packets - entry->retrans_packets_snapshot;

			//printf("%15s:%-5d %15s:%-5d\t", src_ip_addr, src_port, dst_ip_addr, dst_port);
			//printf("Ver %-5llu\t", entry->version);
			//printf("%u/%u ", out_bytes, out_packets);
			//printf("(%u/%u) ", retrans_bytes, retrans_packets);
			//printf("(%u) ", entry->srtt_usec);
			//printf("\n");

			struct Ipv4TcpFlowEntry* export_entry =
					(struct Ipv4TcpFlowEntry*)malloc(sizeof(struct Ipv4TcpFlowEntry));;
			export_entry->src_ip = entry->src_ip;
			export_entry->dst_ip = entry->dst_ip;
			export_entry->src_port = entry->src_port;
			export_entry->dst_port = entry->dst_port;
			export_entry->out_bytes = out_bytes;
			export_entry->out_packets = out_packets;
			export_entry->retrans_bytes = retrans_bytes;
			export_entry->retrans_packets = retrans_packets;
			export_entry->srtt_usec = entry->srtt_usec;
			export_entry->prev = NULL;
			export_entry->next = NULL;

			if (flow_stats_list_front == NULL) {
				flow_stats_list_front = export_entry;
				flow_stats_list_back = export_entry;
			} else {
				flow_stats_list_back->next = export_entry;
				flow_stats_list_back = export_entry;
			}

			entry_wrapper = entry_wrapper->next;
			if (prev_wrapper == NULL) {
				if (active_flows_front == active_flows_back) {
					active_flows_back = entry_wrapper;
					if (active_flows_back != NULL) {
						printf("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
					}
				}
				active_flows_front = entry_wrapper;
			} else {
				prev_wrapper->next = entry_wrapper;
			}
		} else {
			prev_wrapper = entry_wrapper;
			entry_wrapper = entry_wrapper->next;
		}
	}
	...
	struct timeval ts;
	gettimeofday(&ts, NULL);
	if (CalcDeltaTimevalInMicrosecond(ts, next_export_ts) == 0) {
		update_next_export_ts();

		// Add incremental flow stats for long flows.
		struct FlowEntryWrapper* long_entry_wrapper = active_flows_front;
		struct Ipv4TcpFlowEntry* long_entry = NULL;
		u_int32_t out_bytes, out_packets, retrans_bytes, retrans_packets;
		while (long_entry_wrapper != NULL) {
			long_entry = long_entry_wrapper->entry;
			if (long_entry->version == ss_index) {
				out_bytes = long_entry->out_bytes - long_entry->out_bytes_snapshot;
				out_packets =
						long_entry->out_packets - long_entry->out_packets_snapshot;
				retrans_bytes =
						long_entry->retrans_bytes - long_entry->retrans_bytes_snapshot;
				retrans_packets =
						long_entry->retrans_packets - long_entry->retrans_packets_snapshot;

				long_entry->out_bytes_snapshot = long_entry->out_bytes;
				long_entry->out_packets_snapshot = long_entry->out_packets;
				long_entry->retrans_bytes_snapshot = long_entry->retrans_bytes;
				long_entry->retrans_packets_snapshot = long_entry->retrans_packets;

				struct Ipv4TcpFlowEntry* incremental_export_entry =
					(struct Ipv4TcpFlowEntry*)malloc(sizeof(struct Ipv4TcpFlowEntry));;
				incremental_export_entry->src_ip = long_entry->src_ip;
				incremental_export_entry->dst_ip = long_entry->dst_ip;
				incremental_export_entry->src_port = long_entry->src_port;
				incremental_export_entry->dst_port = long_entry->dst_port;
				incremental_export_entry->out_bytes = out_bytes;
				incremental_export_entry->out_packets = out_packets;
				incremental_export_entry->retrans_bytes = retrans_bytes;
				incremental_export_entry->retrans_packets = retrans_packets;
				incremental_export_entry->srtt_usec = long_entry->srtt_usec;
				incremental_export_entry->prev = NULL;
				incremental_export_entry->next = NULL;

				if (flow_stats_list_front == NULL) {
					flow_stats_list_front = incremental_export_entry;
					flow_stats_list_back = incremental_export_entry;
				} else {
					flow_stats_list_back->next = incremental_export_entry;
					flow_stats_list_back = incremental_export_entry;
				}
			}

			long_entry_wrapper = long_entry_wrapper->next;
		}

		if (flow_stats_list_front != NULL) {
			struct Ipv4TcpFlowEntry* flow_stats_list_pending_report =
					flow_stats_list_front;
			flow_stats_list_front = NULL;
			flow_stats_list_back = NULL;

			pthread_t my_thread;
			pthread_create(
					&my_thread, NULL, export_flow, (void*)flow_stats_list_pending_report);
			pthread_join(my_thread, NULL);
		}
	}
}

void process_flow(int sig) {
	tcp_show(&current_filter);
	//render();
	buf_free_all();
	maybe_prepare_export();
	ss_index++;

	ualarm(INTERVAL_SS, 0);
	signal(SIGALRM, process_flow);
}
...
int main(int argc, char *argv[])
{
	...
	while ((ch = getopt_long(argc, argv,
				 "dhaletuwnro460spbEf:j:z:x:miA:D:F:vVZN:KHSO",
				 long_opts, NULL)) != EOF) {
		switch (ch) {
		case 'j':
			...
			printf("Export interval set to %d(sec)+%d(usec)\n",
	...
	if (collector_addr != NULL || collector_port > 0) {
		if (collector_addr != NULL && collector_port > 0) {
			...
			fprintf(stderr, "Incomplete Collector Addr, both IP and port needed\n");
			return 0;
		}
	}
