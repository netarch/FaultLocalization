...
class Flow {
  public:
    ...
    Flow(int src_, int srcport_, int dest_, int destport_, int nbytes_,
         double start_time_ms_);

    //! TODO
    Flow(Flow &flow, unordered_map<Link, Link> &reduced_graph_map,
     ...
     * It's assumed that the snapshots will be accessed in non-decreasing order
     * of time
     */
    bool AnySnapshotBefore(double finish_time_ms);
    void AddSnapshot(double snapshot_time_ms, int packets_sent,
    ...
    */
    ...
    /* To accelerate bayesian net computations, cache expensive intermediate
     * calculation */
