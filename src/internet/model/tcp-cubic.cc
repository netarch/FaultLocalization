    ...
    m_cWndCnt(0)
{
  ...
  //std::cout<<"TcpCubic constructor"<<std::endl;
  ...
  if (count == m_retxThresh && !m_inFastRec)
    {
      ...
      Time m = m_lastRtt;
      //std::cout<<t<<" : "<<&t<<" "<<m_endPoint->GetLocalAddress()<<" "<<m_endPoint->GetPeerAddress()<<" ::: ";
      m_lostPackets++;
      if(!m.IsZero() && m < ns3::MicroSeconds (300)){
          m_randomlyLostPackets++;
          //std::cout<<"m_retxThresh: "<<m_retxThresh<<", breached, possibly random loss "<<m<<std::endl;;
      }
      else if(!m.IsZero()){
          //std::cout<<"m_retxThresh: "<<m_retxThresh<<", breached, possibly congestion loss "<<m<<std::endl;;
      }
